...
const Home = () => {
  ...
  useEffect(() => {
    const fetchInitialCalendarData = async () => {
      try {
        ...
        if (user && accessToken) {
          // Step 1: Fetch calendar-related data first (this must be sequential)
    ...
    const fetchOtherData = async () => {
      try {
        ...
        const sectionDataPromises = sectionId
          ? [fetchSubjectList(), fetchStudentExamSchedule(), fetchRankings()]
          : [];

        await Promise.all([...commonDataPromises, ...sectionDataPromises]);
      } catch (error) {
  ...
  const getClosestExamSchedule = examData => {
    ...
    upcomingTerms.sort(
      (a, b) => a.lastExamDate.diff(today) - b.lastExamDate.diff(today),
    );
  ...
  const fetchStudentExamSchedule = async () => {
    try {
      ...
      if (response?.data?.status === 'success') {
        ...
        console.error(
          'Failed to fetch student exam schedule:',
    ...
    try {
      ...
      if (response?.data?.status === 'success') {
        ...
        if (data) {
          ...
          // console.log('log of student section id', sectionId);
        } else {
          Alert.alert('Error', 'No class details found.');
        }
        //console.log(data); // Debugging
  ...
  const fetchSubjectList = async () => {
    try {
      ...
      // console.log('payload of subject list', payload);
  ...
  const fetchLatestAnnouncement = async () => {
    try {
      ...
      const payload = {
        date: date, // Example date, replace dynamically if needed
  ...
  const fetchRankings = async () => {
    ...
    try {
      const response = await api.protected.post(
        'teacher/progress/hall-of-excellence',
        payload,
      );
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.userInfo}>
          ...
          <Text style={styles.subtext}>
            {t('admission_no')}: {studentClassoff.registration_no} | {t('class')}{' '}
      ...
      <View style={styles.section}>
        <View style={styles.buttonContainer}>
          ...
          <View style = {{maxWidth: moderateScale(250)}}>
          <TouchableOpacity
            onPress={() => navigation.navigate('StudentTimetable')}>
            <Text style={styles.viewDetailSubjectText} numberOfLines={2} ellipsizeMode="tail">{t('view_timetable')}</Text>
          </TouchableOpacity>
          </View>
        </View>
        <ScrollView
      ...
      <View style={styles.ExamHeaderContainer}>
        <Text style={styles.sectionTitle}>{t('exam_schedule')}</Text>
        {examSchedule &&
          examSchedule.subjects &&
          examSchedule.subjects.length > 0 ? (
          <TouchableOpacity
            onPress={() => navigation.navigate('StudentExamSchedule')}>
            <Text style={styles.viewDetailAttendanceText}>{t('view_detail')}</Text>
          </TouchableOpacity>
        ) : null}
      </View>
              ...
              <View key={index} style={styles.classCard}>
                ...
                <View style={styles.detailsRow}>
                  ...
                  <Text style={styles.classDetails}>
                    {moment(subject.start_time, 'HH:mm:ss').format('hh:mm A')} -{' '}
                    {moment(subject.end_time, 'HH:mm:ss').format('hh:mm A')}
                  </Text>
                </View>
                <View style={styles.detailsRow}>
                  <Icon
      ...
      <View style={styles.attendanceSection}>
        ...
        <View style={styles.attendanceLegendContainer}>
          <Text style={styles.attendanceLegendText}>
            <Text style={styles.greenDot}>●</Text> {t('present')}
          </Text>
          <Text style={styles.attendanceLegendText}>
            <Text style={styles.redDot}>●</Text> {t('absent')}
          </Text>
        </View>
          ...
          <AnimatedCircularProgress
            ...
            {fill => (
              <View style={styles.circularTextContainer}>
                ...
                <Text style={styles.attendanceSubText}>{t('total_attendance')}</Text>
              </View>
            )}
          </AnimatedCircularProgress>
        </View>
        ...
        <Text style={styles.attendanceFooterText}>
          {attendanceData.attendance_percentage}% {t('attendance_encouragement')}
        </Text>
      </View>
      ...
      <View style={styles.hallOfExcellenceContainer}>
        ...
        <View style={styles.hallOfExcellenceHeader}>
          <Text style={styles.title}>{t('hall_of_excellence')}</Text>
          <TouchableOpacity
            onPress={() => navigation.navigate('HallOfExcellence')}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          {rankings
            ...
            .map(item => (
              <View
                key={`${item.student_id}-${item.rank}`}
                style={styles.hallOfExcellenceCard}>
                <View style={styles.imageContainer}>
                  ...
                  {getRankSymbol(item.rank) && (
                    <Image
                      source={getRankSymbol(item.rank)}
                ...
                <Text style={styles.achieverDescription}>
                  {t('consistent_high_quality_performance')}
                </Text>
              </View>
            ))}
        </ScrollView>
      ...
      <View style={styles.section}>
        <View style={styles.attendanceSectionHeaderContainer}>
          ...
          <TouchableOpacity
            ...
            <Text style={styles.viewDetailAnnouncementText}>{t('see_all')}</Text>
          </TouchableOpacity>
        </View>
      ...
      <View style={styles.headerContainer}>
        <Text style={styles.sectionTitle}>{t('fees')}</Text>
        <TouchableOpacity onPress={() => navigation.navigate('FeesHistory')}>
          <Text style={styles.viewDetailText}>{t('history')}</Text>
        </TouchableOpacity>
      </View>

      <View style={styles.feeSection}>
        <View style={styles.tableHeader}>
          <Text style={styles.tableHeaderText}>{t('month')}</Text>
          <Text style={styles.tableHeaderText}>{t('amount')}</Text>
          <Text style={styles.tableHeaderText}>{t('due_date')}</Text>
      ...
      <View style={styles.section}>
        <View style={styles.academicSectionHeaderContainer}>
          <Text style={styles.sectionTitle}>{t('academic_calendar')}</Text>
          <View style = {{maxWidth: moderateScale(120)}}>
          <TouchableOpacity
            onPress={() => navigation.navigate('AcademicCalendar')}>
            <Text style={styles.viewDetailCalendarText} numberOfLines={2} ellipsizeMode="tail">{t('view_detail')}</Text>
          </TouchableOpacity>
          </View>
        </View>
        <Calendar
      ...
      <View style={styles.learningCoursesSection}>
        <Text style={styles.learningSectionTitle}>{t('start_learning_today')}</Text>
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          <View style={styles.card}>
            <Icon name="bolt" size={24} color="#000" style={styles.cardIcon} />
            <Text style={styles.cardDate}>Jan 1 - Mar 30, 2025</Text>
            <Text style={styles.cardTitle}>{t('mastering_artificial_intelligence')}</Text>
            <Text style={styles.cardDetails}>
              👨‍🏫 Dr. Sarah Thompson{'\n'}🕒 2:00 PM - 3:30 PM
            </Text>
            <TouchableOpacity onPress={() => navigation.navigate('CourseDetails')}>
              <Text style={styles.cardLink}>{t('view_details')} →</Text>
            </TouchableOpacity>
          </View>
      ...
      <View style={styles.section}>
        <View style={styles.performanceHeader}>
          <Text style={styles.sectionTitle}>{t('monthly_performance')}</Text>
          <TouchableOpacity
            onPress={() => navigation.navigate('StudentProgress')}>
            <Text style={styles.viewDetailCalendarText}>{t('view_details')}</Text>
          </TouchableOpacity>
        </View>
        <View style={styles.progressBarContainer}>
          <Text style={styles.progressLabel}>{t('attendance')}</Text>
        ...
        <View style={styles.progressBarContainer}>
          <Text style={styles.progressLabel}>{t('test')}</Text>
          <View style={styles.progressBarBackground}>
            <View
              style={[
                styles.progressBar,
        ...
        <View style={styles.progressBarContainer}>
          <Text style={styles.progressLabel}>{t('exam')}</Text>
          <View style={styles.progressBarBackground}>
            <View
              style={[
                styles.progressBar,
                {
                  width: `${Number(studentProgress.exam_percentage)}%`,

...
const TeachersRequestEmail = () => {
  ...
  const handleRequestEmail = async () => {
    ...
    try {
      ...
      const response = await api.public.post('request-email', {email, type});

      // console.log('Response:', response.data);

      if (response.data.status === 'success') {
        Alert.alert('Code has been sent to your email');
        navigation.navigate('TeachersVerifyOTP', {email, type});
        ...
        <View style={styles.inputContainer}>
          ...
          {/* <View style={styles.passwordContainer}>
            ...
            <TouchableOpacity
              ...
              <Icon
                name={showPassword ? 'eye-slash' : 'eye'} // Toggle icon
                size={24}
                color="#333333"
              />
            </TouchableOpacity>
...
export default TeachersRequestEmail;

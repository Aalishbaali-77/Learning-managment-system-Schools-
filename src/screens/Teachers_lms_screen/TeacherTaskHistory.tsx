...
const TeacherTaskHistory: React.FC<SubjectsProps> = (props) => {
  ...
  const fetchTaskHistory = async () => {
    ...
    try {
      ...
      const response = await api.protected.post('teacher/assign-tasks', payload);
    ...
    <View style={styles.taskRow}>
      ...
      <Text
        style={[
          styles.taskText,
          styles.column,
          { color: item.assign_task_status === 1 ? '#28A745' : '#FF0000' },
        ]}
      >
        {item.assign_task_status === 1 ? 'Completed' : 'Pending'}
      </Text>
      <TouchableOpacity
        style={[styles.column, { alignItems: 'flex-end' }]}
        onPress={() =>
          navigation.navigate('TeacherTaskHistoryDetail', { taskId: item.id, subjectName:subjectName })
        }
      >
        <Icon name="chevron-right" size={moderateScale(18)} color="#000" style={styles.detailbutton} />
      </TouchableOpacity>
    </View>
  );
  ...
  return (
    <View style={styles.container}>
      ...
      {/* <View style={styles.dateContainer}>
        <Icon name="calendar" size={moderateScale(20)} color="#000" />
        <Text style={styles.dateText}>4 Dec 2024</Text>
      </View> */}

      <View style={styles.summaryContainer}>
        <Text style={styles.subjeectsummaryText}> Subject: {subjectName}</Text>
        <Text style={styles.summaryText}> Total Tasks: {tasks.length}</Text>
        <Text style={styles.summaryText}>
          Completed Tasks: {tasks.filter(task => task.assign_task_status === 1).length} Pending Tasks: {tasks.filter(task => task.assign_task_status !== 1).length}
        </Text>
      </View>
...
const styles = ScaledSheet.create({
  ...
  summaryText: {
    ...
    alignSelf: 'center',
    marginBottom: '8@s', // Corrected to use 's' for scaling
  },
  subjeectsummaryText: {
    fontSize: '15@ms', // Corrected to use 's' for scaling
    color: '#000',
  ...
  taskText: {
    fontSize: '10@ms', // Corrected to use 's' for scaling
    color: '#000',
    fontFamily: 'Poppins-Regular',
    textAlign: 'left', // Align text to the left
  },
  column: {
    flex: 1, // Adjust width equally for all columns
    justifyContent: 'center', // Center text vertically
    alignItems: 'flex-start', // Align text to the start of each column
  ...
  tableHeader: {
    ...
    left: '12@s', // Corrected to use 'ms' for scaling
  ...
  detailbutton: {
    right: '35@s', // Corrected to use 'ms' for scaling
  },
});



export default TeacherTaskHistory;

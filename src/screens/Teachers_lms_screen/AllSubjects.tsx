...
import { ClassItem, ClassSubjects } from '../../types';
import { getAccessToken } from '../../utils/storage';
import { Dropdown } from 'react-native-element-dropdown';

type SubjectsProps = NativeStackScreenProps<any, any>;

const AllSubjects: React.FC<SubjectsProps> = props => {
    ...
    const [selectedClass, setSelectedClass] = useState(null); // Selected sectionId
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                if (user && accessToken) {
                    ...
                    // Fetch other data only when sectionId is available
                    if (selectedClass) {
                        await fetchSubjects(selectedClass); // Pass sectionId to fetchSubjects
                        // console.log('Fetching exams for section:', selectedClass);
                        await fetchExams(selectedClass); // Pass sectionId to fetchExams
                    }
                }
            } catch (error) {
        ...
        fetchData();
    }, [user, selectedClass,]); // Dependency array with 'user' and 'selectedSectionId'

    // Fetch class list and set sectionId (selectedSectionId)
            ...
            if (response?.data?.status === 'success') {
                ...
                if (data && Object.keys(data).length > 0) {
                    const classListArray = Object.values(data).map((item: any) => ({
                        value: item.section_id, // sectionId
                        label: `${item.class_name}-${item.section_name}`,
                    }));
                    setClassList(classListArray); // Set the classList
                    if (!selectedClass) {
                        setSelectedClass(classListArray[0]?.value); // Set the first sectionId if not already selected
                    }
                }
            }
        } catch (error) {
    ...
    const fetchSubjects = async (sectionId: any) => {
        try {
          const response = await api.protected.post(
            'teacher/assign-tasks/subjectList',
            {
              ...
              section_id: sectionId, // Use the passed sectionId
            },
          );
      
          if (response.data.success) {
            setTeacherSubjects(response.data.data); // Update the subjects list
          } else {
      ...
      const fetchExams = async (sectionId: any) => {
        try {
            const payload = {
            ...
            // console.log('Fetching exams with payload:', payload);
            ...
            if (response?.data?.status === 'success') {
                ...
                // If no exams exist, set teacherExams to empty array
                if (!data || data.length === 0) {
                    // console.log('No exams found, clearing the exam list');
                    setTeacherExams([]); // Reset state when no exams
                } else {
                    // console.log('Exams found:', data);
                    setTeacherExams(data); // Update state with fetched exams
                }
            } else {
           ...
           // console.error('Error fetching exams:', error);
            setTeacherExams([]); // Clear exams list if API fails
        }
    };
            ...
            <View style={styles.container}>
                ...
                <View style={styles.classSection}>
                    {/* <View style={styles.dropDownContainer}> */}
                        <Dropdown
                            data={classList} // Populated by fetchClassesList
                            labelField="label" // Corresponds to the `label` field in classList
                            valueField="value" // Corresponds to the `value` field in classList
                            value={selectedClass} // The selected item (controlled by state)
                            placeholder="Select Class-Section"
                            onChange={item => {
                                setSelectedClass(item.value); // Save selected sectionId
                                fetchSubjects(item.value);
                                fetchExams(item.value);
                            }}
                            ...
                            placeholderStyle={{ color: '#fff', fontSize: moderateScale(10) }}
                            labelStyle={{ color: '#fff', fontSize: moderateScale(10) }}
...
const styles = ScaledSheet.create({
    ...
    dropdown: {
        ...
        marginBottom: '10@vs', // Use fixed margin, no scaling needed

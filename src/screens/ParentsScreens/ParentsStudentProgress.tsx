...
const ParentsStudentProgress = () => {
    const { user } = useUser();
       ...
       const fetchSubjectList = async () => {
           try {
            ...
            //    console.log('Section ID of API:', sectionId);
               ...
               //('Error fetching subject list:', error);
           }
       };
       ...
       const fetchStudentsTestHistory = async () => {
           ...
           try {
               ...
               if (response?.data?.success) {
                   ...
                   //console.error('Failed to fetch student test history:', response?.data?.message);
               }
           } catch (error) {
       ...
       const fetchExamTermList = async () => {
           try {
            ...
            //    console.log('Payload for API:', payload);
               ...
               const response = await api.protected.post('student/examTermTypeList', payload);
            ...
            //    console.log('Full API Response:', response);
               ...
               if (response?.data?.status === 'success') {
                ...
                //    console.log('Formatted Exam Term List for Dropdown:', formattedExamTermList);
       ...
       const fetchExamProgress = async () => {
           try {
            ...
            //    console.log('Payload for fetching student exam progress:', payload);
               ...
               // Make the API call
               const response = await api.protected.post('teacher/progress/examProgress', payload);
            ...
            //    console.log('Full API Response for Exam Progress:', response);
               ...
               if (response?.data?.status === 'success') {
                  ...
                  // console.error('API returned an error:', response?.data?.message || 'Unexpected error');
               }
           } catch (error) {
       ...
       useEffect(() => {
           const fetchStudentsExamProgressData = async () => {
               if (!selectedTerm) {
                //    console.log('No term selected, skipping fetch');
                   return; // Skip if no subject is selected
               }
       ...
       const renderExam = ({ item }) => (
           <View style={styles.examTaskRow}>
               ...
               <Text style={styles.taskStatusText}>{item.grade !== null ? item.grade : 'N/A'}</Text>
           </View>
       );
           ...
           <View style={styles.container}>
               ...
               <ScrollView showsVerticalScrollIndicator={false}>
                   ...
                   <View style={styles.monthSummary}>
                       <Text style={styles.monthText}>January Present: {monthAttendanceData?.present}</Text>
                       <Text style={styles.monthText}>January Absent: {monthAttendanceData?.absent}</Text>
                       <Text style={styles.monthText}>January Late: {monthAttendanceData?.late}</Text>
                   </View>
                   ...
                   <View style={styles.dropDownContainer}>
                       ...
                       <Dropdown
                           ...
                           itemTextStyle={{ color: '#000', fontSize: moderateScale(10), fontFamily: 'Poppins-Regular' }}
                   ...
                   <View style={styles.testProgressBarContainer}>
                       ...
                       <Text style={styles.progressLabel}>{studentProgress?.combined_percentage}%</Text>
                       <View style={styles.progressBarBackground}>
                           <View
                               style={[
                                   styles.progressBarBottom,
                                   { width: `${Number(studentProgress?.combined_percentage) || 0}%`, backgroundColor: 'green' },
                               ]}
                           />
                       </View>
                   ...
                   <View style={styles.examProgressBarContainer}>
                       ...
                       <Text style={styles.progressLabel}>{studentProgress?.exam_percentage}%</Text>
                       <View style={styles.progressBarBackground}>
                           <View
                               style={[
                                   styles.progressBarBottom,
                                   { width: `${Number(studentProgress?.exam_percentage) || 0}%`, backgroundColor: 'green' },
                               ]}
                           />
                       </View>
                       <Text style={styles.progressLabelPercentage}>100%</Text>
   ...
   const styles = ScaledSheet.create({
       ...
       summaryCard: {
           ...
           borderRadius: '10@ms', // Width scaling (border radius)
           padding: '20@ms', // Width scaling (padding)
       ...
       summaryTitle: {
           fontSize: '16@ms', // Width scaling (font size)
       ...
       dropdownText: {
           fontSize: '14@ms', // Width scaling (font size)
           fontFamily: 'Poppins-Regular',
           color: '#000',
           marginRight: '5@s', // Universal scaling (for spacing)
       },
       summaryDetails: {
           flexDirection: 'row',
           justifyContent: 'space-between',
           alignItems: 'center',
           marginBottom: '10@vs', // Vertical scaling (margin)
       },
       summaryText: {
           fontSize: '14@ms', // Width scaling (font size)
           fontFamily: 'Poppins-Regular',
       ...
       progressBar: {
           height: '10@vs', // Vertical scaling (height)
           borderRadius: '5@ms', // Width scaling (border radius)
           backgroundColor: '#e0e0e0',
           marginBottom: '10@vs', // Vertical scaling (margin)
       },
       progressFill: {
           width: '69%', // Use percentage for width, doesn't need scaling
       ...
       attendanceCounts: {
           flexDirection: 'row',
           justifyContent: 'space-between',
           marginHorizontal: '10@s', // Universal scaling (for horizontal spacing)
           right: '10@s', // Universal scaling (for positioning)
       },
       countText: {
           fontSize: '12@ms', // Width scaling (font size)
           fontFamily: 'Poppins-Regular',
           color: '#000',
       },
       attendanceCountsNumber: {
           flexDirection: 'row',
           justifyContent: 'space-between',
           marginHorizontal: '20@s', // Universal scaling (horizontal margin)
           right: '15@s', // Universal scaling (positioning)
       },
       countTextNumber: {
           fontSize: '12@ms', // Width scaling (font size)
           fontFamily: 'Poppins-Regular',
       ...
       statusTitle: {
           ...
           color: '#000',
           marginBottom: '10@vs', // Vertical scaling (margin)
           top: '10@vs', // Vertical scaling (position)
       },
       statusTags: {
           flexDirection: 'row',
           justifyContent: 'space-between',
           margin: '45@ms', // Width scaling (margin)
           bottom: '70@vs', // Vertical scaling (position)
           left: '20@s', // Width scaling (position)
       },
       statusTag: {
           borderRadius: '5@ms', // Width scaling (border radius)
           paddingHorizontal: '15@s', // Universal scaling (horizontal padding)
           paddingVertical: '5@vs', // Vertical scaling (vertical padding)
       },
       statusText: {
       ...
       calendar: {
           borderRadius: '10@ms', // Width scaling (border radius)
           marginBottom: '20@vs', // Vertical scaling (margin)
           bottom: '90@ms', // Width scaling (position)
       },
       examHeaderSection: {
       ...
       sectionTitle: {
           fontSize: '18@ms', // Width scaling (font size)
           fontFamily: 'Poppins-Bold',
           color: '#000',
       ...
       tableHeaderText: {
           fontSize: '12@ms', // Universal scaling (font size)
...
export default ParentsStudentProgress;
